#!/bin/bash
#
# Themis CLI binary installation script
# ------------------------------------
#
...
CYN  "Themis CLI binary installation script"
echo "---------------------------------------"
echo ""
...
case "$PROCESSOR" in
    arm* | aarch* | ppc* )
        if [ "$OS_FLAVOUR" != Darwin ]; then
            ...
            echo "  => $(CYN https://github.com/metaplex-foundation/themis)"
            echo ""
            echo "to build Themis from the source code."
            exit 1
        fi
        ;;
...
RELEASE_URL="https://github.com/metaplex-foundation/themis/releases"
RELEASE="latest"
BIN="themis"
...
if [ $SIZE -eq 0 ]; then
    RED "Aborting: could not download Themis distribution"
    exit 1
fi
...
if [ ! "$(command -v $BIN)" = "" ]; then
    ...
    echo "Themis binary was found at:"
    echo "  => $(CYN $EXISTING)"
    echo ""
    ...
    if [ "$(command -v $BIN)" = "" ]; then
        ...
        if [ -f "$ENV_FILE" ]; then
            ...
            echo "  => adding '$TARGET' to 'PATH' variable to execute 'themis' from any directory."
...
# sanity check
if [ "$(command -v $BIN)" = "" ]; then
    # installation was completed, but Themis is not in the PATH
